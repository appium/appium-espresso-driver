...
import { copyRecursively } from './utils';


const TEST_SERVER_PATH = path.resolve(__dirname, '..', '..', 'espresso-server');
...
const REQUIRED_PARAMS = ['adb', 'tmpDir', 'host', 'systemPort', 'devicePort', 'appPackage', 'forceEspressoRebuild', 'espressoBuildConfig'];
...
class EspressoRunner {
  ...
  async buildNewModServer () {
    ...
    if (this.espressoBuildConfig) {
      ...
      try {
        versions = JSON.parse(await fs.readFile(this.espressoBuildConfig, 'utf8'));
      } catch (e) {
    ...
    logger.debug(`Copying espresso server template from (${TEST_SERVER_PATH} to ${serverPath})`);
    await copyRecursively(TEST_SERVER_PATH, serverPath);
    await new ServerBuilder({serverPath, versions, showGradleLog: this.showGradleLog}).build();
