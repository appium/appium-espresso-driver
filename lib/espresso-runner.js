...
class EspressoRunner {
  ...
  async installTestApk () {
    if (!this.forceEspressoRebuild && await this.isAppPackageChanged()) {
      logger.info(`Forcing espresso server rebuild because of changed application package`);
      this.forceEspressoRebuild = true;
    }
    ...
    if (this.forceEspressoRebuild && await this.adb.uninstallApk(TEST_APK_PKG)) {
