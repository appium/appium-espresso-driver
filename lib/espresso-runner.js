...
import { buildServerSigningConfig } from './server-builder';
...
class EspressoRunner {
  constructor (opts = {}) {
    ...
    if (opts.useKeystore) {
      if (opts.keystorePath && opts.keystorePassword && opts.keyAlias && opts.keyPassword) {
        this.signingConfig = buildServerSigningConfig(opts.keystorePath, opts.keystorePassword, opts.keyAlias, opts.keyPassword);
      }
    }
  }
  ...
  async buildNewModServer () {
    ...
    await new ServerBuilder({serverPath, buildConfiguration, showGradleLog: this.showGradleLog}).build(this.appPackage, this.signingConfig);
