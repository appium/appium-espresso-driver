...
function buildServerSigningConfig (keystoreFile, keystorePassword, keyAlias, keyPassword) {
  let json = {};
  json.zipAlign = true;
  json.keystoreFile = keystoreFile;
  json.keystorePassword = keystorePassword;
...
class ServerBuilder {
  ...
  async build (testAppPackage, signingConfig) {
    ...
    await this.runBuildProcess(testAppPackage, signingConfig);
  }

  getCommand (testAppPackage, signingConfig) {
    ...
    let args = VERSION_KEYS
      ...
      .map(key => {
        ...
        return this.buildGradleProperty(gradleProperty, serverVersion);
      })
      .filter(Boolean);

    if (signingConfig) {
      args.push(this.buildGradleProperty('appiumZipAlign', signingConfig.zipAlign));
      args.push(this.buildGradleProperty('appiumKeystoreFile', signingConfig.keystoreFile));
      args.push(this.buildGradleProperty('appiumKeystorePassword', signingConfig.keystorePassword));
      args.push(this.buildGradleProperty('appiumKeyAlias', signingConfig.keyAlias));
      args.push(this.buildGradleProperty('appiumKeyPassword', signingConfig.keyPassword));
    }

    if (testAppPackage) {
      args.push(this.buildGradleProperty('appiumTargetPackage', testAppPackage));
    }
  ...
  async runBuildProcess (testAppPackage, signingConfig) {
    const {cmd, args} = this.getCommand(testAppPackage, signingConfig);
