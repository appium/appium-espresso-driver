...
class ServerBuilder {
  ...
  constructor (args = {}) {
    ...
    const versions_arg = args.versions || {};
    const server_versions = {};
    for (const key of VERSION_KEYS) {
      if (versions_arg[key]) {
        server_versions[key] = versions_arg[key];
      }
    }
  ...
  runBuildProcess () {
    ...
    gradlebuild._last_lines = [];

    const logGradleOutput = this.showGradleLog;
    const logMsg = _.isBoolean(this.showGradleLog)
      ? `Output from Gradle ${this.showGradleLog ? 'will' : 'will not'} be logged`
      : 'Output from Gradle will only be logged if any errors are present there';
    ...
    gradlebuild.on('output', (stdout, stderr) => {
      ...
      for (const line of out.split(EOL)) {
        ...
        gradlebuild._last_lines.push(`${EOL}${line}`);
        if (gradlebuild._last_lines.length > GRADLE_MAX_ERROR_LOG_LINES) {
          gradlebuild._last_lines = gradlebuild._last_lines.slice(-GRADLE_MAX_ERROR_LOG_LINES);
        }
      }
    });
    ...
    return new B((resolve, reject) => {
      gradlebuild.on('exit', (code, signal) => {
        ...
        gradlebuild.processExited = true;
        if (!signal && code !== 0) {
          return reject(new Error(`Gradle build failed with code ${code}${EOL}` +
            `Gradle error message:${EOL}${gradlebuild._last_lines}`));
        }
