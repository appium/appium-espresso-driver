...
const DEFAULT_ADDITIONAL_DEPENDENCIES = {
...
class ServerBuilder {
  ...
  defaultDependencies (additionalAppDependencies) {
    const dependencies = DEFAULT_ADDITIONAL_DEPENDENCIES;

    for (const dependencyName in dependencies) {
      for (const additionalAppDependency of additionalAppDependencies) {
        if (additionalAppDependency.includes(dependencyName)) {
  ...
  updateDependencyLines (configurationContent, additionalAppDependencies) {
    const dependencyLines = [
      ...
      ..._.reduce(this.defaultDependencies(additionalAppDependencies), (acc, depVer, depName) => acc.concat(`${depName}:${depVer}`), [])
