...
class ServerBuilder {
  ...
  async insertAdditionalDependencies () {
    ...
    for (const propName of ['additionalAppDependencies', 'additionalAndroidTestDependencies']) {
    ...
    for (const propName of ['additionalAppDependencies', 'additionalAndroidTestDependencies']) {
      const prefix = propName === 'additionalAppDependencies'
        ? 'implementation'
      ...
      log.info(`Adding the following ${propName} to build.gradle: ${deps}`);
