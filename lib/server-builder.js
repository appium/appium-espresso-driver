...
class ServerBuilder {
  constructor (args = {}) {
    ...
    this.serverVersions = Object.entries(versionConfiguration)
      .filter(function ([key]) {
  ...
  async build () {
    ...
    if (this.additionalAppDependencies.length > 0) {
    ...
    const dependencyLines = additionalAppDependencies
      .map(function (dependency) {
        if (/[\s']/.test(dependency)) {
          throw new Error(`Single quotes and space characters are disallowed in additional dependencies: ${dependency}`);
        }
