 ...
 * - target can be either 'activity' or 'application'. If 'activity', it will try to invoke given method on current activity
...
commands.mobileBackdoor = async function (opts = {}) {
  const {target, methods} = assertRequiredOptions(opts, ['target', 'methods']);
  return await this.espresso.jwproxy.command(`/appium/execute_mobile/backdoor`, 'POST', {target, methods});
};


/**
 *  API to invoke methods defined on element views.
 *
 *  Example data
 *{
 *  "element": "1d737327-280d-4837-af5f-deacb0d002da",
 *  "methods": [
 *    {
 *      "name": "someMethod"
 *    },
 ...
 *  ]
 *}
 * In above example, method "someMethod" will be invoked on element with specified id. On the result, "anotherMethod" will be invoked
 * - Only Public methods can be invoked
 ...
 *
 */
commands.mobileElementInvoke = async function (opts = {}) {
  const {element, methods} = assertRequiredOptions(opts, ['element', 'methods']);
  return await this.espresso.jwproxy.command(`/appium/execute_mobile/${util.unwrapElement(element)}/invoke_method`, 'POST', {methods});
};
