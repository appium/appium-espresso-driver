...
// Eg: To invoke a single method called foo(), opts will be
//      [{name:"foo"}]
// If method takes parameters eg foo(String lol, String bar), opts should be
//      [{name:"foo", args:["lol","bar"]}]
// Methods can be chained together with opts looking like
//      [{name:"foo", args:["lol","bar"]}, {name:"qux"}]
commands.mobileBackdoor = async function (opts = {}) {
  opts.forEach(function (value) {
    if (!value.hasOwnProperty("name")) {
      throw new Error(`"name" key missing in: ${JSON.stringify(value)}`);
    }
  });
  return await this.espresso.jwproxy.command(`/appium/execute_mobile/backdoor`, 'POST', { opts });
};
