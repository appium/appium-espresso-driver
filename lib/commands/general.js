...
commands.uiautomator = async function (opts = {}) {
  const {byMethod, value, action} = assertRequiredOptions(opts, ['byMethod', 'value', 'action']);
  const {index} = opts;
  return await this.espresso.jwproxy.command(`/appium/execute_mobile/uiautomator`, 'POST', {byMethod, value, index, action});
};
