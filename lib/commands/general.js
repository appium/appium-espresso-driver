 ...
 * Example: de
 * @property {?string} country An optional country name.
 * Example: CA
 * @property {?string} variant On optional name of the language subscript,
 * Example: Hans
 *
 * Complete locale names in Android must follow BCP-47. Read
 * https://github.com/libyal/libfwnt/wiki/Language-Code-identifiers
 * for more details.
 */

/**
 * Changes locale for the target application context
 *
 * @param {?Locale} opts
 * @throws {Error} if it is not possible to parse the given locale
 * or Espresso fails to set it
 */
commands.mobileSetAppLocale = async function mobileSetAppLocale (opts = {}) {
  const {language, country, variant} = assertRequiredOptions(opts, ['language']);
  await this.espresso.jwproxy.command(`/session/:sessionId/appium/execute_mobile/locale`, 'POST', {
    language,
    country,
    variant
  });
