...
commands.uiautomator = async function (opts = {}) {
  const {strategy, value, action, index} = assertRequiredOptions(opts, ['strategy', 'value', 'action']);
  return await this.espresso.jwproxy.command(`/appium/execute_mobile/uiautomator`, 'POST', {strategy, value, index, action});
};
