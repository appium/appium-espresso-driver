...
class EspressoDriver extends BaseDriver {
  ...
  async startEspressoSession () {
    ...
    if (!this.caps.appWaitPackage) {
      this.caps.appWaitPackage = appInfo.appWaitPackage || appInfo.appPackage;
    }
    ...
    if (this.caps.appWaitActivity) {
      this.caps.appWaitActivity = qualifyActivityName(this.caps.appWaitActivity, this.caps.appPackage);
    } else {
      this.caps.appWaitActivity = qualifyActivityName(appInfo.appWaitActivity || appInfo.appActivity,
        this.caps.appPackage);
    }

    // launch espresso and wait till its online and we have a session
    await this.espresso.startSession(this.caps);

    await this.adb.waitForActivity(this.opts.appWaitPackage, this.opts.appWaitActivity, this.opts.appWaitDuration);
