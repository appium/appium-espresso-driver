...
class EspressoDriver extends BaseDriver {
  ...
  async setWindowAnimationState(animationState) {
    const isAnimationOn = await this.adb.isAnimationOn();
    const shouldDisableAnimation = animationState && isAnimationOn;
    const shouldEnableAnimation = !animationState && !isAnimationOn;
    ...
    if (shouldDisableAnimation) {
      ...
      this.log.debug(`Window animation is already ${animationState ? 'disabled' : 'enabled'}`);
    }
  }
