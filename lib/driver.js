  ...
  async createSession (...args) {
    try {
      ...
      if (this.opts.app) {
        ...
        // Check that it has the INTERNET permission set. This is required to use espresso-server.
        const adb = await ADB.createADB();
        if (!await adb.hasInternetPermissionFromManifest(this.opts.app)) {
          return logger.errorAndThrow(`Espresso Server requires the 'INTERNET' permission to be set in the Android manifest (<uses-permission android:name="android.permission.INTERNET" />)`);
        }
