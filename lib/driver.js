...
class EspressoDriver extends BaseDriver {
  ...
  async startEspressoSession () {
    ...
    this.caps.setAnimation = this.caps.setAnimation || false;
    // https://github.com/appium/appium-espresso-driver/issues/72
    if (this.caps.setAnimation && await this.adb.isAnimationOn()) {
      this.log.debug('Animation is already on');
      this.wasAnimationEnabled = true;
    } else if (this.caps.setAnimation && (await this.adb.isAnimationOn() === false)) {
      try {
        await this.adb.setAnimationState(true);
        this.wasAnimationEnabled = false;
      } catch (err) {
        this.log.warn(`Unable to turn on animations: ${err.message}`);
      }
    } else if (this.caps.setAnimation === false && await this.adb.isAnimationOn()) {
      try {
