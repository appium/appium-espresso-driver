...
class EspressoDriver extends BaseDriver {
  ...
  async startEspressoSession () {
    ...
    this.caps.disableWindowAnimation = this.caps.disableWindowAnimation !== false;
    // https://github.com/appium/appium-espresso-driver/issues/72
    await this.setWindowAnimationState(this.caps.disableWindowAnimation);
  ...
  async setWindowAnimationState(disableAnimation) {
    const isAnimationOn = await this.adb.isAnimationOn();
    const shouldDisableAnimation = disableAnimation && isAnimationOn;
    const shouldEnableAnimation = !disableAnimation && !isAnimationOn;
    ...
    if (shouldDisableAnimation) {
      ...
      this.log.debug(`Window animation is already ${disableAnimation ? 'disabled' : 'enabled'}`);
    }
  }
