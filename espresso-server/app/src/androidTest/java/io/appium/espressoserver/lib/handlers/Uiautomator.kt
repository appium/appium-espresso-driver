...
class Uiautomator : RequestHandler<UiautomatorParams, List<Any?>> {
    ...
    override fun handleInternal(params: UiautomatorParams): List<Any?> {
        ...
        try {
            val byMethod = KReflectionUtils.extractMethod(By::class.java, params.strategy.methodName, String::class.java)
            val bySelector = KReflectionUtils.invokeMethod(null, byMethod, locator) as BySelector
            val actionMethod = KReflectionUtils.extractMethod(UiObject2::class.java, params.action.actionName)
            ...
            index ?: run {
                return uiObjects.map {
                    KReflectionUtils.invokeMethod(it, actionMethod)
                }
            }
            ...
            return listOf(KReflectionUtils.invokeMethod(uiObjects[index], actionMethod))
