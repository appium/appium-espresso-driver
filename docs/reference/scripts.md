...
will be able to reuse the already-build server application. Available since driver version 2.18.0.
...
|--|--|--|--|
|`--build-config`|Path to a JSON build configuration file. Must match the format of the `appium:espressoBuildConfig` capability. Can alternatively be specified using the `ESPRESSO_BUILD_CONFIG` environment variable.|string||
...
and suggests fragments for the `appium:espressoBuildConfig` capability. Available since driver
version 8.6.0.
...
Prints the path to the Espresso server directory. Available since driver version 2.15.2.
